        ...
        <div class="mt-3" v-for="qnafreeanswer in qnafreeanswer.data" :key="qnafreeanswer">
			...
			<div v-if="isUpdateFormVisible[qnafreeanswer.freeCommentNum]" >
				<form class="comment-form update-form" @submit.prevent="answerupdate(qnafreeanswer.freeCommentNum)">
					<input type="hidden" name="freeCommentRefGroup" :value="qnafreeanswer.freeQuestionNum"/>
...
export default {
	...
	methods: {
		...
		paging: function (currentPage) {
			var url2 = "/project/api/qna-free-answer/list";
			const data = {
				limit: 10,
		...
		submitAnswerForm(){
			...
			const data = {
				...
				freeCommentRefGroup : this.formData.freeCommentRefGroup
			};
			axios.post(url, data)
			.then(response => {
				console.log(response.data);
