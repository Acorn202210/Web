			...
			<div v-for="faq in faq.data" :key="faq">
				...
				<div class="accordion-item">
					...
					<div class="accordion-body">
						...
						<div><textarea readonly style="border:none; height:200px" v-model="faq.content"></textarea></div>
					</div>
					</div>
				</div>
				</div>
...
export default {
	...
	methods: {
		...
		faqone : function(faqNum) {
			axios.get('/plec/api/faq/' + faqNum + '/faqOne',
				{ faqNum }
			).then((response) => {
		...
		faqupdatedelete : function(faqNum) {
			axios.put('/plec/api/faq/' + faqNum + '/delete', 
				{ faqNum }
			).then(response => {
				console.warn(response)
				this.result = response.data;
